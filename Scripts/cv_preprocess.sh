...
# Specific cases: Line splitting for CRTVG (lowercase, full stop-no space-, uppercase)
#sed -i -r 's/([a-záéíóúñ]\.)([A-ZÁÉÍÓÚÑ])/\1\n\2/g' "${filename}_lines.txt"

# Replace all cases of double punctuation ("?." "!." ".," ",?" ",!")
...
grep -E '\.{3,}[a-zA-ZáéíóúñÁÉÍÓÚÑ]+' "${filename}_lines_14.txt" > "${filename}_ellipsis.txt"
sed -i -r '/\.{3,}[a-zA-ZáéíóúñÁÉÍÓÚÑ]+/d' "${filename}_lines_14.txt"

# Specific cases: Delete lines ending in lowercase dash
#sed -i '/[a-záéíóúñ]-$/d' "${filename}_lines_14.txt"
...
grep -E '[0-9]' "${filename}_lines_14.txt" > "${filename}_numbers.txt"
...
#grep -v -f "${filename}_odd_quotes.txt" "${filename}_lines_14.txt" > "${filename}_prep.txt"
...
grep -v -f "${filename}_odd_quotes_escaped.txt" "${filename}_lines_14.txt" > "${filename}_prep.txt"
...
grep -E '^.*[^\.!\?:\"]$' "${filename}_prep.txt" > "${filename}_finalpunct.txt"
sed -i -r '/^.*[^\.!\?:\"]$/d' "${filename}_prep.txt"

# Delete blank lines from the preprocessed file
sed -i '/^[[:space:]]*$/d' "${filename}_prep.txt"

# Leading and trailing space/tab deletion
sed -i 's/^[ \t]*//' "${filename}_prep.txt"
sed -i 's/[ \t]*$//' "${filename}_prep.txt"

# Delete repeated lines from the preprocessed file
awk '!seen[$0]++' "${filename}_prep.txt" > "${filename}_temp.txt"
mv "${filename}_temp.txt" "${filename}_prep.txt"

# Check again that all sentences are 14 words or fewer (if neccessary)
#grep -E '\b(\w+\b\s+){14,}\w+[.?!]' "${filename}_prep.txt" > longer.txt
...
mv "${filename}_lines.txt" "${filename}_lines_14.txt" "${filename}_ellipsis.txt" "${filename}_abbrev.txt" "${filename}_numbers.txt" "${filename}_odd_quotes.txt" "${filename}_finalpunct.txt" "$output_folder/"

# Move preprocessed file to specific folder
mkdir -p "OUTPUT/"
mv "${filename}_prep.txt" "OUTPUT/"
